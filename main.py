...
def create_models(features, window_size, batch_size):
    ...
    for i in range(features.get_qtd_of_features()):
        model = Lstm(input_shape=(features.get_batch_size(), window_size, features.get_feature_size()), index=i, batch_size=batch_size, window_size=window_size)
        model.create_model()
        models.append(model)
...
def create_array_of_features_and_labels(train_size, list_of_features, forecast, window_size, batch_size):
    features = Feature(train_size, list_of_features, forecast, window_size, batch_size)
...
def loop(features, models, results):    
    while not features.has_ended():
        ...
        for result in results:
            train_data, train_label, validation_data, validation_label, test_data, ground_truth, scaler = features.get_data(result.get_feature_position())
            
            models[result.get_feature_position()].fit(train_data, train_label, validation_data, validation_label)
            
            predicted = models[result.get_feature_position()].predict(test_data)
...
def main(args):
    ...
    features = create_array_of_features_and_labels(args.train_size, list_of_features, args.forecast, args.window_size, args.batch_size)
...
if __name__=='__main__':
    ...
    parser.add_argument('-b', action="store", type=int, default=10, dest='batch_size')
